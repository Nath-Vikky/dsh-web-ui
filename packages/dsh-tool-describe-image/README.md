...
| Capability route | `GET /describe-image/capability?session=<id>` answers whether the session's model declares image input (only the live agent/request record confirms the effective route; modalities resolve through `resolveModelInfo`, and a model selection change clears the record). Every unrecorded session, unknown, and failure answers false, preserving the rewrite behavior |
