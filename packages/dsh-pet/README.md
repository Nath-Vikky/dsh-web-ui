...
| Inner whispers | 碎碎念: while the model streams, the pet occasionally speaks its inner voice through its own bubble — a fresh whisper takes over the display session's bubble and re-tints it violet (「」 quotes included) instead of stacking a second bubble — keyword moods woken by the model output (errors, test greens, plans, victories...) plus ambient whispers earned by output volume; paced by a cooldown, the status copy returns after a few seconds |
...
| Desktop ergonomics | Scale choices are limited to 100%–200% to keep the sprite uncropped; the hover controls open toward the available vertical space, concurrent sessions collapse behind the primary bubble's +N badge, a fresh inner whisper re-tints and takes over that same bubble, all bubble UI overlays the sprite without enlarging its window, and quitting from the tray also turns off the persisted desktop switch |
...
Desktop imports are validated and copied into the same custom-pet directory with both `pet-model.json` and `pet.json`; the desktop list updates immediately, and the Web selector sees the import after that restart. Existing managed desktop imports are copied forward without deleting their old files.
...
|       `-- pet.module.css
|-- desktop/                 # optional managed Electron presentation
...
optional legacy activity/status ------------------/
                         ...
                         /api/pet/* JSON              authenticated loopback SSE
                                      |                         |
global React root (browser, polling 2s)             Electron desktop presentation
```
...
```sh
pnpm build          # host/browser bundle + managed Electron presentation
pnpm test           # Host/Web tests, then Electron main/renderer tests
