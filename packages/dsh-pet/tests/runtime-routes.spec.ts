...
import type { ElectronRuntimeManager, ElectronRuntimeView } from '../src/runtime/electron-runtime.ts'

const missing: ElectronRuntimeView = {
  version: '43.4.0',
  platform: 'win32',
  arch: 'x64',
  phase: 'not-installed',
...
describe('desktop runtime routes', () => {
  ...
  it('exposes status, install and cancellation only to the loopback Web UI', async () => {
    const runtime = {
      state: vi.fn(() => missing),
      startInstall: vi.fn(() => ({ ...missing, phase: 'downloading', source: 'npmmirror' })),
      cancelInstall: vi.fn(() => missing),
    } as unknown as ElectronRuntimeManager
