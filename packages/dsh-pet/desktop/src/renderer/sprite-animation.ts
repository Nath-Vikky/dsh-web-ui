import type { PetAnimation, PetMotion, PetSnapshot } from '../shared/desktop-api.ts'
...
const motionAnimation: Record<PetMotion, SpriteAnimation> = {
  idle: 'idle',
  'look-around': 'waiting',
  thinking: 'running',
...
export function animationForPetSnapshot(snapshot: PetRenderSnapshot | null | undefined): SpriteAnimation {
  if (snapshot?.intent !== undefined) return motionAnimation[snapshot.intent.motion]
  return snapshot?.animation ?? 'idle'
}
