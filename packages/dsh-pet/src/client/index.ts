...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(() => {
    const disposers: Array<() => void> = [
      ctx.on('connection/reset', () => { void settingsScope.load() }),
    ]
    const remote = ctx.get('remote', false) as unknown
    if (typeof remote === 'object' && remote !== null && typeof (remote as { $on?: unknown }).$on === 'function') {
      disposers.push((remote as { $on: (event: string, callback: (namespace?: unknown) => void) => () => void })
