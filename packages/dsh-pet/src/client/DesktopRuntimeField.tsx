...
async function runtimeRequest(path = '', body?: unknown): Promise<RuntimeView> {
  ...
  if (!response.ok) {
    throw new Error('error' in value && typeof value.error === 'string' ? value.error : 'runtime-request-failed')
  }
...
/** Desktop switch plus first-use, checksum-verified Electron runtime installation. */
export function DesktopRuntimeField(props: DesktopRuntimeFieldProps) {
  ...
  const install = async (): Promise<void> => {
    ...
    try {
      const next = await runtimeRequest('/install', {
        source,
    ...
    <div className={switchCss.field}>
      <div className={switchCss.head}>
        <span>
          ...
          <span className={css.runtimeStatus} role="status">{status}</span>
        </span>
        <span className={switchCss.actions}>
          {props.overridden
            ? (
              <button type="button" className={switchCss.reset} disabled={props.disabled || busy} onClick={props.onReset}>
                {props.resetLabel}
              </button>
            )
            : null}
      ...
      {dialogOpen
        ? (
          <div className={css.backdrop}>
            <section className={css.dialog} role="dialog" aria-modal="true" aria-labelledby="pet-runtime-title">
              <header className={css.dialogHeader}>
                <div>
                  <h3 id="pet-runtime-title">{props.t('settings.runtimeDialogTitle')}</h3>
                  <p>{props.t('settings.runtimeDialogDescription', { version: runtime?.version ?? '' })}</p>
                </div>
              </header>
              ...
              {error !== undefined
                ? <p className={css.error} role="alert">{error === 'runtime-enable-failed' ? props.t('settings.runtimeErrorEnable') : runtimeErrorCopy(props.t, error)}</p>
                : null}
