/** Pinned Electron runtime artifacts. Mirrors are untrusted; these hashes are authoritative. */
export const ELECTRON_RUNTIME_VERSION = '43.4.0'

export const ELECTRON_RUNTIME_CHECKSUMS: Readonly<Record<string, string>> = {
...
export type ElectronRuntimePlatform = 'win32' | 'darwin' | 'linux'
export type ElectronRuntimeArch = 'x64' | 'arm64'

export function electronRuntimeArtifact(
  platform: NodeJS.Platform,
  arch: string,
): { platform: ElectronRuntimePlatform, arch: ElectronRuntimeArch, filename: string, checksum: string } | undefined {
  if (platform !== 'win32' && platform !== 'darwin' && platform !== 'linux') return undefined
  if (arch !== 'x64' && arch !== 'arm64') return undefined
  const filename = `electron-v${ELECTRON_RUNTIME_VERSION}-${platform}-${arch}.zip`
  const checksum = ELECTRON_RUNTIME_CHECKSUMS[filename]
